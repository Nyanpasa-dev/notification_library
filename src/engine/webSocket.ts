...
import { Client, Gateway, MessageData, SendMessageParams } from '../types';
...
class MyGateway implements Gateway {
    ...
    private setupWebSocketServer(port?: number): void {
        ...
        port = port ? port : parseInt(process.env.WB_PORT || '4461', 10);


        this.server = this.createServer(isProduction);
