...
describe('NotificationManager', () => {
  ...
  it('should send bulk immediate notifications', async () => {
    ...
    await expect(notificationManager.bulkBroadcastEmmediatelyNotification(data)).resolves.toBeInstanceOf(NotificationManager);
  });
  ...
  it('should send bulk delayed notifications', async () => {
    ...
    await expect(notificationManager.bulkBroadcastDelayedNotification(data, 5000)).resolves.toBeInstanceOf(NotificationManager);
  });
  ...
  it('should throw an error if receivers list is empty for immediate notification', async () => {
    ...
    await expect(notificationManager.broadcastEmmediatelyNotification(data)).rejects.toThrow('Receivers list is empty.');
  });
  ...
  it('should throw an error if delay is invalid for delayed notification', async () => {
    ...
    await expect(notificationManager.broadcastDelayedNotification(data)).rejects.toThrow('Delay is invalid.');
  });
